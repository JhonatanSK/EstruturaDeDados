...
public class BinaryTree {
	...
	public String getTreePosOrdered(Node node, int level) {
		String	output = "";
		
		if(node != null) {
			output += getTreePosOrdered(node.getLeft(), level+1);					
			output += getTreePosOrdered(node.getRight(), level+1);	
			output += node.getElement() + " ";
