...
public class BinaryTree {
	...
	public String getTreePreOrdered(Node node, int level) {
		String	output = "";
		
		if(node != null) {
			output += node.getElement() + " ";
			output += getTreePreOrdered(node.getLeft(), level+1);					
			output += getTreePreOrdered(node.getRight(), level+1);	
